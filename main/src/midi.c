    ...
    if(xQueueReceive(xMidiMsgHandle, (void *)&msg, (TickType_t)portMAX_DELAY)){
      ...
      switch(msg_id){
        case NOTE_OFF:
          ...
          lifo_remove(&mc.active_notes, msg.f1);
          ESP_LOGD(TAG, "active_note=%d", mc.active_notes.buf[mc.active_notes.curr_pos]);
          if(mc.active_notes.buf[mc.active_notes.curr_pos] != 0){
            mc.pitch = MidiPitchMap[mc.active_notes.buf[mc.active_notes.curr_pos]];
          }
        ...
        case NOTE_ON:
          ...
          lifo_push(&mc.active_notes, msg.f1);
